...
case class DatafinitiAPIv3(apiToken: String) extends DatafinitiAPI with LazyLogging {
  ...
  implicit val loggerImp: Logger = logger
  ...
  private def request[T](uri: String, followRedirect: Boolean = true)(codeCheck: Int => Boolean, successHandler: HttpResponse[String] => DatafinitiResponse[T])(implicit ec: ExecutionContext): DatafinitiFuture[T] = {
    EitherT(
      Future({
        ...
        .map(response => {
          ...
          response.code match {
            ...
            case code => Left(DatafinitiError.WrongHttpResponseCode(response.code, response.body, safeUri(uri)))
          }
        })
        .recover {
          // $COVERAGE-OFF$Not sure how to test this
          case t: Throwable => {
            Left(DatafinitiError.APICallFailed(t.getMessage, safeUri(uri)))
          }
          // $COVERAGE-ON$
        }
    )
  ...
  def download(apiView: APIView, query: Option[String] = None, format: APIFormat = JSON): Unit = { //: Future[Either[Throwable, String]] = {
    ...
    //          logger.debug(s"Found redirect from ${safeUri(uri)} to => $redirect")
    //          Right(redirect)
    //        }
    //        case None => {
