...
/**
  ...
  */
class UnexpectedDownloadStatusTest extends FunSuite {

  val e = UnexpectedDownloadStatus("status", "data", "http://datlinq.com")

  test("testException") {
    assert(e.exception.getMessage.contains(e.data))
    assert(e.exception.getMessage.contains(e.url))
    assert(e.exception.getMessage.contains(e.status))
  }
